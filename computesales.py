...
def similar(a, b):
    ...
    Returns:
        float: The similarity ratio between the strings.
...
def compute_total_cost(price_catalogue, sales_record):
    ...
    Args:
        price_catalogue (list): A list of products with prices.
        sales_record (list): A list of sales records.
    ...
    for sale in sales_record:
        ...
        if not match_found:
            ...
            if match_found:
                for catalog_product in price_catalogue:
                    if product_name.lower() == catalog_product["title"].lower():
                        price = catalog_product["price"]
                        quantity = sale["Quantity"]
                        total_cost += price * quantity
                        break
            else:
                print(f"Warning: No match found for product '{product_name}' in price catalogue.")

    return total_cost

def main():
    """
    Main function to compute total cost of sales.
    """
